...
const form = useForm({
    ...
    wants: props.listing.wants ?? '',
    budget: props.listing.budget ?? null,
    description: props.listing.description ?? '',
    quantity: props.listing.quantity ?? 1,
    photos: [],          // nouvelles photos ajoutées
    remove_photos: [],   // identifiants des photos existantes à retirer
});

// Photos déjà en ligne, que le vendeur peut retirer une par une.
const existing = ref([...(props.listing.photos ?? [])]);
function removeExisting(photo) {
    form.remove_photos = [...form.remove_photos, photo.id];
...
function onPhotos(e) {
    ...
    const room = MAX_PHOTOS - form.photos.length - existing.value.length;
...
function submit() {
    form.post(`/livres/${props.listing.id}`, { forceFormData: true });
}

function destroy() {
    if (confirm(`Supprimer définitivement « ${props.listing.title} » ? Cette action est irréversible.`)) {
        router.delete(`/livres/${props.listing.id}`);
    }
}
...
<template>
    ...
    <PublicLayout>
        <div class="max-w-3xl mx-auto px-4 py-10">
            <div class="flex items-center gap-2 text-sm text-gray-500 font-medium mb-5">
                <Link href="/dashboard" class="hover:text-brand-600">Tableau de bord</Link>
                <i class="fa-solid fa-chevron-right text-[10px] text-gray-300"></i>
                <Link :href="`/livres/${listing.id}`" class="hover:text-brand-600 truncate max-w-[220px]">{{ listing.title }}</Link>
                <i class="fa-solid fa-chevron-right text-[10px] text-gray-300"></i>
                <span class="text-dark font-semibold">Modifier</span>
            </div>

            <h1 class="text-3xl font-black text-dark mb-2">Modifier l'annonce</h1>
            <p class="text-gray-500 font-medium mb-8">Corrigez les informations, ajustez le prix ou changez les photos.</p>
            ...
            <form @submit.prevent="submit" class="space-y-8">
                ...
                <!-- Photos -->
                <div class="bg-white rounded-2xl border border-gray-200 p-6">
                    <h2 class="font-bold text-dark mb-1">Photos <span class="text-gray-400 font-medium text-sm">({{ existing.length + form.photos.length }}/10)</span></h2>
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
                        ...
                        <div v-for="(p, i) in existing" :key="p.id" class="relative group aspect-square rounded-xl overflow-hidden border border-gray-200">
                            <img :src="`/storage/${p.path}`" class="w-full h-full object-cover" :alt="`Photo ${i + 1}`">
                            <span v-if="i === 0 && !form.photos.length" class="absolute bottom-0 inset-x-0 bg-brand-600/90 text-white text-[10px] font-bold text-center py-0.5">Principale</span>
                            <button type="button" @click="removeExisting(p)"
                        ...
                        <label v-if="existing.length + form.photos.length < 10"
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-5">
                        ...
                        <div v-if="form.type !== 'recherche'">
                            <label class="block text-sm font-bold text-dark mb-2">Exemplaires disponibles</label>
                            <input v-model="form.quantity" type="number" min="1" max="999"
                                   class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-200 transition-all">
                            <p class="text-xs text-gray-400 mt-1">Décrémenté à chaque remise confirmée.</p>
                ...
                <div class="flex flex-col sm:flex-row gap-3 sm:items-center">
                    <button type="button" @click="destroy"
                            class="text-center text-red-600 border-2 border-red-100 hover:border-red-300 hover:bg-red-50 px-5 py-3.5 rounded-full font-bold transition-colors">
                        <i class="fa-solid fa-trash-can text-sm"></i> Supprimer l'annonce
                    </button>
                    <Link :href="`/livres/${listing.id}`" class="sm:ml-auto text-center bg-white text-dark border-2 border-gray-200 px-6 py-3.5 rounded-full font-bold hover:border-dark transition-colors">Annuler</Link>
                    <button type="submit" :disabled="form.processing" class="bg-brand-600 hover:bg-brand-700 text-white px-8 py-3.5 rounded-full font-bold shadow-floating transition-all active:scale-95 disabled:opacity-60 flex items-center justify-center gap-2">
                        ...
                        Enregistrer <i v-if="!form.processing" class="fa-solid fa-check text-xs"></i>
                    </button>
                </div>
            </form>
        </div>
